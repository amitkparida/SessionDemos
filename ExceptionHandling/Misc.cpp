...
// Important Notes:
//
// SEH handler __except(EXCEPTION_EXECUTE_HANDLER) can catch exceptions raised by RaiseException API, e.g. RaiseException(EXCEPTION_CODE_1, 0, 2, NULL); 
// Hardware exptions are also caught by __except(EXCEPTION_EXECUTE_HANDLER). 
// Hardware exptions are not thrown/raised by RaiseException or throw.
// Hardware exptions can not be caught by C++ catch(...). 
// By default, C++ catch() can catch exceptions thrown by C++ throw.
